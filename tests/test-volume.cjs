...
(async()=>{
 ...
 for(const sid of ['s0327','s1456','s1438']){
  ...
  for(const fraction of [.2,.5,.8]){
   run(`pos[2]=Math.floor(meta.shape[2]*${fraction});render()`);
  ...
  if(sid==='s0327')assert(new Set(sets).size>1,'Organ list must follow slice');
  for(let p=0;p<3;p++){
